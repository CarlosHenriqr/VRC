...
import { Code2, Smartphone, Workflow, Brain, Compass } from "lucide-react";
        ...
        <div className="mt-16 grid overflow-hidden rounded-2xl border border-hairline sm:grid-cols-2 lg:grid-cols-3">
          {services.map((s, i) => (
            <motion.div
              ...
              className="group relative -mr-px -mb-px border-r border-b border-hairline bg-background p-8 transition-colors duration-300 hover:bg-cloud"
            >
              ...
              <p className="mt-3 text-[0.95rem] leading-relaxed text-muted-foreground">{s.desc}</p>
            </motion.div>
          ))}
        </div>
      </div>
